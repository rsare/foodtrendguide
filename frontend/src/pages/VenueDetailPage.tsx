...
function VenueDetailPage() {
    ...
    useEffect(() => {
        ...
        // 2. Yorumları Çek
        if(id) {
            axios.get(`http://localhost:8081/api/reviews/venue/${id}`)
    ...
    const handleSubmitReview = async () => {
        ...
        try {
            await axios.post(`http://localhost:8081/api/reviews/${userId}/${id}`, {
                text: newReview,
                rating: newRating.toString()
            });

            // Yorumları yenile
            const res = await axios.get(`http://localhost:8081/api/reviews/venue/${id}`);
            setReviews(res.data);
